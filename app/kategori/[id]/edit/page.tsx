...
const Edit = () => {
    ...
    useEffect(() => {
        const id = params.id;
        if (id) {
            axios.get(`http://localhost:8000/api/kategori/${id}`)
                .then(response => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (id) {
            try {
                await axios.put(`http://localhost:8000/api/kategori/${id}`, { nama, deskripsi});
    ...
    return (
        <div className="min-h-screen bg-gray-100 py-8 px-4">
            <div className="max-w-4xl mx-auto bg-white shadow-lg rounded-lg overflow-hidden">
                <div className="p-6">
                    <h1 className="text-2xl font-semibold text-gray-800 mb-6">Edit Kategori</h1>
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div>
                            <label className="block text-gray-700 text-sm font-medium mb-1" htmlFor="nama">Nama Kategori</label>
                            <input
                                id="nama"
                                type="text"
                                value={nama}
